...
import { normalizeStructLogs, isCallLike, isEndOpcode, getAddressFromStackEntry } from "./utils";
...
export function getTracesByContractAddress(structLogs: StructLog[], startAddress: string): TraceByContractAddress {
    ...
    for (let i = 0; i < normalizedStructLogs.length; i++) {
        ...
        if (isCallLike(structLog.op)) {
            ...
            if (nextStructLog.depth !== structLog.depth) {
                ...
                traceByContractAddress[currentAddress] = (
                    traceByContractAddress[currentAddress] || []
                ).concat(currentTraceSegment);
                currentTraceSegment = [];
            }
        } else if (isEndOpcode(structLog.op)) {
    ...
    if (currentTraceSegment.length !== 0) {
        ...
        currentTraceSegment = [];
        logUtils.warn(
            "Malformed trace. Current trace segment non empty at the end"
        );
    }
    return traceByContractAddress;
}
